...
\lstdefinestyle{soliditystyle}{
	...
	\author{
		...
		High distribution costs, weak trust in insurersan d limited actuarial data block traditional coverage.
		...
		Premiums and yields are set through a seasonal sealed-bid uniform-price auction functioning as a prediction market: farmer bids and investor offers jointly reveal the market-implied drought probability $\hat{\pi}$ and equilibrium margin~\parencite{hanson2003market,wolfers2004prediction}.
		The frontend abstracts blockchain logic and integrates fiat--stablecoin conversion.
		...
		\end{enumerate}
		If (1)--(3) hold, the contract transfers $qL$ (or pro-rata if rationing applies) from the vault to the claimant’s wallet; otherwise the call reverts.
		Operational costs $F$ are deducted before distribution.
        ...
        The design couples a sealed-bid, uniform-price mechanism for seasonal price discovery with tokenized coverage and a pooled ERC-4626 vault, and a test-covered prototype was implemented that executes end-to-end flows with deterministic, auditable logic.
